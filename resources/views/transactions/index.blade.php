...
@section('content')
{{-- Tambahkan ini agar JS bisa membaca tipe bisnis tanpa merubah struktur --}}
...
<div class="grid grid-cols-1 md:grid-cols-12 gap-4">
    ...
    <div class="md:col-span-8 bg-white p-4 rounded-xl shadow-sm">
        ...
        <div class="space-y-3 text-sm">
            @forelse ($transactions as $trx)
                <div class="grid grid-cols-11 items-center border-b pb-2">
                    ...
                    <div class="col-span-3 text-gray-600 text-sm">
                        {{ \Carbon\Carbon::parse($trx->transaction_date)->translatedFormat('l, d F Y') }}
                    </div>
                    ...
                    <div class="col-span-2 flex justify-end gap-2 whitespace-nowrap">
                        ...
                        @endif
                        <button type="button" class="text-gray-400 hover:text-blue-600 transition" data-id="{{ $trx->id }}" data-type="{{ $trx->type }}" data-category="{{ $trx->category }}" data-amount="{{ $trx->amount }}" data-date="{{ $trx->transaction_date }}" data-note="{{ e($trx->note) }}" data-image="{{ $trx->image ?? '' }}" onclick="openEdit(this)">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @empty
                <p class="text-center text-gray-500 text-sm">Tidak ada transaksi</p>
            @endforelse
        </div>
...
<div id="editModal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-50">
    <div class="bg-white rounded-xl max-w-md w-full p-4 relative">
        ...
        <form id="editForm" method="POST" enctype="multipart/form-data" class="space-y-3 text-sm">
            ...
            <div>
                <label class="block mb-1 font-medium">Kategori</label>
                <select id="editCategory" name="category" class="w-full border rounded-md p-2"></select>
            </div>
            <div>
                <label class="block mb-1 font-medium">Jumlah (Rp)</label>
                <input id="editAmount" name="amount" type="number" class="w-full border rounded-md p-2">
            </div>
            <div>
                <label class="block mb-1 font-medium">Tanggal</label>
                <input id="editDate" name="transaction_date" type="date" class="w-full border rounded-md p-2">
            </div>
            <div>
...
<script>
// AMBIL BUSINESS TYPE DARI AUTH UNTUK FILTERING
const businessType = "{{ auth()->user()->business_type }}";

// PEMETAAN KATEGORI BERDASARKAN SKALA UMKM
const categories = {
    income: (function() {
        if (businessType === 'Mikro') {
            return ["Penjualan Produk", "Lainnya"];
        } else if (businessType === 'Kecil') {
            return ["Penjualan Produk", "Penjualan Jasa", "Hadiah/Bonus", "Lainnya"];
        } else {
            // Skala Menengah menampilkan semua kategori pendapatan
            return ["Penjualan Produk", "Penjualan Jasa", "Investasi", "Sewa Properti", "Royalti", "Bunga Bank", "Hadiah/Bonus", "Lainnya"];
        }
    })(),
    expense: [
...
// LOGIKA DROPDOWN TAMBAH TRANSAKSI
